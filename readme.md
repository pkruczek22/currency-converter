# Curency Converter project

## Description
This web project was created as part of the Frontend Web Developer course. Web is a currency converter, where you can check the amount after conversion by the exchange rate of selected currency.

<br>

## 🕹️ Demo
Try this link 👉 https://pkruczek22.github.io/currency-converter/

<br>

## Take a look at my website 😀
![website gif](https://github.com/pkruczek22/currency-converter/blob/main/images/readme.gif?raw=true)

<br>


## Tech stack:
- HTML
- JAVASCRIPT
- CSS
- BEM
- Markdown

<br>

## Would you like to see more? Check this out 👇
💻 Homepage Project 👉 https://github.com/pkruczek22/homepage

<br> 

*... More and more coming soon!* 🤩